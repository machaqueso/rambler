...
namespace Rambler.Services
{
    public class IntegrationService
    {
        ...
        public async Task Create(Integration integration)
        {
            if (integration == null)
            {
            ...
            {
                throw new UnprocessableEntityException($"Integration name is required");
            }

            if (await db.Integrations.AnyAsync(x => x.Name == integration.Name))
            {
                throw new ConflictException($"Integration named '{integration.Name}' already exists.");
            }
        ...
        {
            var entity = await GetIntegrations().SingleOrDefaultAsync(x => x.Id == id);
